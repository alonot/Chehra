...
def draw_face(result_location, frame):
    # to store the actual frame to send it to capture_pic function.
    actual_frame = copy.copy(frame)
    ...
    if result_location:
        for face_loc in result_location:
            ...
            if capturing > 0:
                ...
                frame, pic_no, capturing = embed.capture_pic(
                    actual_frame, pic_no, capturing)
                if ctime-ptime < 1:
...
def display_process_info(frame):
    global match, recognize_time, capturing, ctime, ptime, vid

    # If person is recognized:
    if match is not None:
        cv.rectangle(frame, (left_point[0], right_point[1]),
                     (right_point[0], right_point[1]+25), (140, 4, 90), cv.FILLED)
        cv.putText(frame, f"Match Found: {match}", (
            left_point[0]+10, right_point[1]+15), cv.FONT_HERSHEY_SCRIPT_SIMPLEX, 0.45, (104, 154, 196), 1)
        capturing = 0
        ...
        if recognize_time >= 5.4:
            # backup of the frame to display a different message if required.
            backup_frame = frame

            # If capturing is not yet started then ask the user to start it by pressing 'c'
            if capturing == 0:
                cv.putText(frame, "Hold 'c' to register"+("."*count),
                           (left_point[0], left_point[1]-5), cv.FONT_HERSHEY_DUPLEX, 0.37, (0, 0, 255), 1)
            ...
            if capturing > 0:
                frame = cv.putText(backup_frame, "Registering. PLease be Still"+("."*count),
                                   (left_point[0], left_point[1]-5), cv.FONT_HERSHEY_DUPLEX, 0.37, (4, 100, 255), 1)

        else:
            cv.putText(frame, "Recognizing"+("."*count),
                       (left_point[0], left_point[1]-5), cv.FONT_HERSHEY_COMPLEX, 0.37, (255, 255, 0), 1)
...
def decorator(img):
    ...
    cv.putText(img, f"Time: {t}", (w-150, 20),
               cv.FONT_HERSHEY_COMPLEX, 0.5, (255, 25, 255), 1)
...
while True:
    ...
    frame = cv.normalize(frame, None, alpha=0, beta=350,
                         norm_type=cv.NORM_MINMAX, dtype=cv.CV_8U)

    small_frame = rescale(frame, 0.25)
    frame = rescale(frame, 1.5)

    cv.putText(frame, f"FPS: {int(fps)}", (20, 20),
    ...
    if (process_frame):
        ...
        if (recognize_time >= 5.0 and start_wait >= 4.0 and capturing == 0):
